...
/// Different display modes of [CustomCupertinoDatePicker].
///
/// See also:
///
...
enum CustomCupertinoDatePickerMode {
  ...
  /// The AM/PM designation is shown only if [CustomCupertinoDatePicker] does not use 24h format.
  ...
  /// Mode that shows the date in year.
  ///
  /// Example: ` 012 `.
  year,
}
...
///
/// There are several modes of the date picker listed in [CustomCupertinoDatePickerMode].
///
...
class CustomCupertinoDatePicker extends StatefulWidget {
  ...
  ///
  /// [mode] is one of the mode listed in [CustomCupertinoDatePickerMode] and defaults
  /// to [CustomCupertinoDatePickerMode.dateAndTime].
  ///
  /// [onDateTimeChanged] is the callback called when the selected date or time
  /// changes and must not be null. When in [CustomCupertinoDatePickerMode.time] mode,
  /// the year, month and day will be the same as [initialDateTime]. When in
  /// [CustomMode.date] mode, this callback will always report the
  /// start time of the currently selected day. When in
  /// [CustomCupertinoDatePickerMode.monthYear] mode, the day and time will be the
  ...
  /// In [CustomCupertinoDatePickerMode.time] mode, [minimumDate] should typically be
  ...
  /// In [CustomCupertinoDatePickerMode.time] mode, [maximumDate] should typically be
  /// on the same date as [initialDateTime], as the picker will not limit the
  ...
  /// [CustomCupertinoDatePickerMode.date] mode. Defaults to 1 and must not be null.
  ///
  /// [maximumYear] is the maximum year that the picker can be scrolled to in
  /// [CustomCupertinoDatePickerMode.date] mode. Null if there's no limit.
  ///
  ...
  ///
  /// [dateOrder] determines the order of the columns inside [CustomCupertinoDatePicker]
  /// in [CustomCupertinoDatePickerMode.date] and [CustomCupertinoDatePickerMode.monthYear]
  ...
  CustomCupertinoDatePicker({
    ...
    this.pickerTextStyle = _kDefaultPickerTextStyle,
  ...
  /// The mode of the date picker as one of [CustomCupertinoDatePickerMode].
  /// Defaults to [CustomCupertinoDatePickerMode.dateAndTime]. Cannot be null and
  ...
  ///
  /// In [CustomCupertinoDatePickerMode.time] mode, a time becomes unselectable if the
  ...
  /// Determines the order of the columns inside [CustomCupertinoDatePicker] in
  /// [CustomCupertinoDatePickerMode.date] and [CustomCupertinoDatePickerMode.monthYear]
  /// mode. When using monthYear mode, both [DatePickerDateOrder.dmy] and
  /// [DatePickerDateOrder.mdy] will result in the month|year order.
  /// Defaults to the locale's default date format/order.
  ...
  State<StatefulWidget> createState() {
    switch (mode) {
      ...
      case CustomCupertinoDatePickerMode.monthYear:
        return _CustomCupertinoDatePickerMonthYearState(dateOrder: dateOrder);
      case CustomCupertinoDatePickerMode.year:
        return _CustomCupertinoDatePickerYearState(dateOrder: dateOrder);
    }
  }

  // Estimate the minimum width that each column needs to layout its content.
  static double _getColumnWidth(_PickerColumnType columnType, CupertinoLocalizations localizations,
      BuildContext context, bool showDayOfWeek) {
    String longestText = '';
...
class _CustomCupertinoDatePickerDateTimeState extends State<CustomCupertinoDatePicker> {
  ...
  void initState() {
    ...
    minuteController =
        FixedExtentScrollController(initialItem: initialDateTime.minute ~/ widget.minuteInterval);
  ...
  double _getEstimatedColumnWidth(_PickerColumnType columnType) {
    if (estimatedColumnWidths[columnType.index] == null) {
      estimatedColumnWidths[columnType.index] = CustomCupertinoDatePicker._getColumnWidth(
  ...
  void _refreshEstimatedColumnWidths() {
    estimatedColumnWidths[_PickerColumnType.dayOfMonth.index] =
        CustomCupertinoDatePicker._getColumnWidth(
            _PickerColumnType.dayOfMonth, localizations, context, widget.showDayOfWeek);
    estimatedColumnWidths[_PickerColumnType.month.index] =
        CustomCupertinoDatePicker._getColumnWidth(
            _PickerColumnType.month, localizations, context, widget.showDayOfWeek);
    estimatedColumnWidths[_PickerColumnType.year.index] = CustomCupertinoDatePicker._getColumnWidth(
...
class _CustomCupertinoDatePickerMonthYearState extends State<CustomCupertinoDatePicker> {
...
class _CustomCupertinoDatePickerYearState extends State<CustomCupertinoDatePicker> {
  _CustomCupertinoDatePickerYearState({
    required this.dateOrder,
  });
    ...
    return NotificationListener<ScrollNotification>(
      ...
      child: CupertinoPicker.builder(
        ...
        onSelectedItemChanged: (int index) {
          selectedYear = index;
          if (_isCurrentDateValid) {
            widget.onDateTimeChanged(DateTime(selectedYear));
          }
        },
        itemBuilder: (BuildContext context, int year) {
          if (year < widget.minimumYear) {
  ...
  bool get _isCurrentDateValid {
    // The current date selection represents a range [minSelectedData, maxSelectDate].
    final DateTime minSelectedDate = DateTime(selectedYear);
    final DateTime maxSelectedDate = DateTime(selectedYear);
  ...
  void _pickerDidStopScrolling() {
    ...
    // Whenever scrolling lands on an invalid entry, the picker
    // automatically scrolls to a valid one.
    final DateTime minSelectDate = DateTime(selectedYear);
    final DateTime maxSelectDate = DateTime(selectedYear);
  ...
  Widget build(BuildContext context) {
    List<_ColumnBuilder> pickerBuilders = <_ColumnBuilder>[];
    double columnWidths = MediaQuery.of(context).size.width - 60;
    final DatePickerDateOrder datePickerDateOrder = dateOrder ?? localizations.datePickerDateOrder;
    ...
    const double offAxisFraction = 0.0;
    ...
    return CupertinoPicker(
      ...
      children: List<Widget>.generate(60 ~/ widget.minuteInterval, (int index) {
        ...
        return Semantics(
          ...
          excludeSemantics: true,
          child:
              _buildPickerNumberLabel(localizations.timerPickerMinute(minute), additionalPadding),
        );
      }),
    );
  }
    ...
    return CupertinoPicker(
      ...
      children: List<Widget>.generate(60 ~/ widget.secondInterval, (int index) {
        ...
        return Semantics(
          ...
          excludeSemantics: true,
          child:
              _buildPickerNumberLabel(localizations.timerPickerSecond(second), additionalPadding),
        );
      }),
    );
  }
